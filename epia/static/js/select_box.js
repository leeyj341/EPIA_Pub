...
$('.sel').each(function() {
  ...
  function setPositions(index) {
    ...
    axios.get('/epia/select_position/' + options[index].getAttribute('value'))
  ...
  function setQuestions(index) {
    ...
    axios.get('/epia/select_question/' + options[index].getAttribute('value'))
      .then(function(response) {
  ...
  const submitBtnEasy = document.querySelector('#start-interview-easy');
  const submitBtnHard = document.querySelector('#start-interview-hard');

  submitBtnEasy.addEventListener('click', function() {
    ...
    sendForm.mode.checked = false;
    sendForm.submit();
  });

  submitBtnHard.addEventListener('click', function() {
    if(childSpanQuestion.innerText.includes('질문을 선택하세요.')) {
      alert('주어진 항목을 모두 선택해주세요!');
      return;
    }
